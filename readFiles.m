...
%                                    particular time => zone(n-1) x zone(n) x time(quantized)
function [TRANSITION,DELTAT]=readFiles(Path,numberOfFilesToRead,clearPreviousData)
...
if clearPreviousData
    TRANSITION = zeros(K_RANDOM,length(ZONE),length(ZONE));
    DELTAT = zeros(K_RANDOM,length(ZONE),length(ZONE),length(QUANTIZATION));
else
